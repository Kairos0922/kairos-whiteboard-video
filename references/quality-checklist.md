...
- [ ] 背景匀净度量：每板背景区局部标准差与中亮灰绿像素占比出数值；灰雾/ smudge 块超阈 = 不合格
- [ ] 用户给了参考图时：板图 vs 参考图强制并排三项比对（色板直方图 / 线宽 / 填色策略），
      差异结论写进确认包；"风格不对"必须落成这三项的差，不许只写感觉
...
## 成片终审（机器闸 `validate --full`，2026-09-18 重写）

> 判据全部落在 `engine/scripts/whiteboard_story/qa_gates.py`；
> `workflow.py validate --full [--sheets-dir DIR]` 一条命令跑完并产出人审接触表。
> 散文终审测不出感知类缺陷（一期实盘：自评 97、回记实为 74；扫荡在 1fps 漏检）。

机器闸（任一不过 = 阻断）：

- [ ] 规格：h264 / yuv420p / 1920×1080 / 30fps + aac 44100 + faststart（moov 在 mdat 前）
- [ ] 响度 −16±0.5 LUFS、真峰 −1.5±0.5 dBTP；silencedetect −35dB/2s 零段
- [ ] 扫荡审计：任务表任意 2s 窗 travel>800ms 且 draw 弧长<60px 的窗数 = 0（手可见地动而墨不长）
- [ ] 切幕：切点窗单帧跳变 <12 且窗累计 >8（700ms 擦黑板淡入、非硬切）；
      切点 +0.8s 残影 ROI y502:930 均差 <24（上一幕墨迹必须擦净）
- [ ] 幕末缺墨：真墨口径（三通道差和 >160）每幕缺墨率 ≤1%（调度不得丢笔画）
- [ ] 词级偏移 ≤120ms（validate 既有护栏）

人审闸（接触表由 `--sheets-dir` 产出；**帧率写死，不许降**）：

- [ ] 首幕 2fps 条带 + 其余各幕 1fps 接触表逐帧看，对照反模式表：扫荡 / 手空转（手在动墨不长）/
      空档见手 / 骷髅脸 / 灰雾 / 残影 / 标签先挂 / 鬼影后换图
- [ ] 有手的帧：同窗笔尖处墨必须增长；无手的帧：板面静止属旁白空档（手离画是正确行为）
- [ ] 底部字幕可读不压内容；总时长与旁白 ±200ms；结尾停留 ≥0.5s

诚实评分卡（**交付信息必须附；禁无评分卡交付**）：

- [ ] 100 分 = 缺陷 40 + 风格 25 + AV 15 + 叙事 10 + 顺滑 10
- [ ] 每扣分附证据帧或闸口输出行；任何轴满分必须有对应人审记录或闸口输出，不许凭印象
- [ ] 评分卡列剩余缺陷清单与解锁路径（如有）；interim 交付仅在用户知情批准时允许
