...
6. **风格块准入硬规则**（缺一项 = 探针不批；一期实盘教训：这两条在期内打回后才补，
   等于整期板图返工一轮）：
   - **角色圣经**：脸的完整定义（发色与发丝线、脸型、眼形与单高光、腮红、嘴），并写明
     任何脸——含图示里的图标小头——必须是完整头；显式禁"只有眼睛的圆框/面具感/
     浮空眼"等骷髅感字形。
   - **背景匀净**：板底必须全幅单一匀净底色；显式禁灰雾/ smudge 块/擦痕/渐晕/渐变。
7. 角色定妆、手笔：脚本需要什么再补 `characters/` 与 `hands/`。手素材分两档
   ...
   程序化生成，但只配 paper 主题。
8. 用同一主题跑通一期短片（至少 2 幕）确认渲染器 paper/chalk 判墨、手部锚点。换手素材必须重校笔尖锚点（`<name>.tip.json` sidecar 优先，缺省才退 `HAND_TIP_RATIO`；见 kernel-design.md）。
9. 项目维护者冻结风格块后写入 registry.json。未冻结的主题禁止用于正式发布期。
