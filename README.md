...
默认 `chalkboard-chibi` 主题以参考板图为冻结合同：统一深墨绿底、暖象牙细线、navy 头发/外套、桃色皮肤、少量 violet/coral/mustard；主题的 `theme.json`、`style-block.txt`、`character-contract.txt`、`probe.json` 在生成前会做同源校验，避免 prompt 与探针再次互相打架。
